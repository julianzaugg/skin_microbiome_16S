...
# Perhaps abundance plots for discrete time groupings, further broken down by transplant type. 
# Also look at the abundance of the most abundant genera vs time since transplant
...
filter_summary_to_top_n <- function(taxa_summary, grouping_variables, abundance_column, my_top_n = 10){
  ...
  out <- 
    ...
    dplyr::top_n(n = my_top_n, get(abundance_column)) %>% 
...
# Length_of_immunosuppression_group_1
# Length_of_immunosuppression_group_2
...
# unique(immunocompromised_data.df$Length_of_immunosuppression_group_1)
# Faceting by Sampletype_final_refined
immunocompromised_data.df$Sampletype_final_refined <- factor(immunocompromised_data.df$Sampletype_final_refined, levels = c("C","C_P", "AK", "SCC_PL", "SCC"))
...
# immunocompromised_data.df$Sampletype_final_refined <- factor(immunocompromised_data.df$Sampletype_final_refined, levels = rev(c("C","C_P", "AK", "SCC_PL", "SCC")))
...
unique(subset(immunocompromised_data.df,Sampletype_final_refined  == "C")$Patient)
immunocompromised_data.df %>% group_by(Length_of_immunosuppression_group_1, Sampletype_final_refined) %>% 
  summarise(number_of_samples = max(n_distinct(Sample)), number_of_patients = max(n_distinct(Patient)))

LOS_group_1_sample_summary.df <- immunocompromised_data.df %>% select(Sample,Patient,Sampletype_final_refined, Length_of_immunosuppression_group_1, Bacterial_load_CFU) %>% 
  unique() %>%
  group_by(Length_of_immunosuppression_group_1, Sampletype_final_refined) %>%
  mutate(number_of_samples = n_distinct(Sample), number_of_patients = n_distinct(Patient))  %>%
  ...
  as.data.frame()
LOS_group_2_sample_summary.df <- immunocompromised_data.df %>% select(Sample,Patient,Sampletype_final_refined, Length_of_immunosuppression_group_2, Bacterial_load_CFU) %>% 
  unique() %>%
  group_by(Length_of_immunosuppression_group_2, Sampletype_final_refined) %>%
  mutate(number_of_samples = n_distinct(Sample),number_of_patients = n_distinct(Patient))  %>%
  summarise(number_of_samples = max(number_of_samples),
            number_of_patients = max(number_of_patients),
            samples_with_bacterial_load_greater_than_zero = sum(Bacterial_load_CFU > 0, na.rm = T),
...
LOI_group_1_summary.df <- generate_taxa_summary(mydata = immunocompromised_data.df, taxa_column = "taxonomy_label", group_by_columns = c("Length_of_immunosuppression_group_1", "Sampletype_final_refined"))
LOI_group_2_summary.df <- generate_taxa_summary(mydata = immunocompromised_data.df, taxa_column = "taxonomy_label", group_by_columns = c("Length_of_immunosuppression_group_2", "Sampletype_final_refined"))

lesion_summary.df <- generate_taxa_summary(mydata = immunocompromised_data.df, taxa_column = "taxonomy_label", group_by_columns = c("Sampletype_final_refined"))
...
#                                                                   grouping_variables = c("Length_of_immunosuppression_group_1","Sampletype_final_refined"),
...
#                                                       grouping_variables = c("Length_of_immunosuppression_group_2","Sampletype_final_refined"),
#                                                       abundance_column = "Mean_relative_abundance_rarefied",
#                                                       my_top_n = 5)
...
# Take the full table and re-label any taxa not in the top taxa for each length of suppression group to "Other"
...
# Take the full table and re-label any taxa not in the top taxa for each lesion group to "Other"
...
# Normalise the Mean_relative_abundance_rarefied values within each length of suppression group
LOI_group_1_summary.df <- LOI_group_1_summary.df %>% group_by(Length_of_immunosuppression_group_1, Sampletype_final_refined) %>% mutate(Normalised_mean_relative_abundance = Mean_relative_abundance_rarefied/sum(Mean_relative_abundance_rarefied)) %>% as.data.frame()
LOI_group_2_summary.df <- LOI_group_2_summary.df %>% group_by(Length_of_immunosuppression_group_2, Sampletype_final_refined) %>% mutate(Normalised_mean_relative_abundance = Mean_relative_abundance_rarefied/sum(Mean_relative_abundance_rarefied)) %>% as.data.frame()
# LOI_group_1_summary.df <- LOI_group_1_summary.df %>% group_by(Length_of_immunosuppression_group_1, Sampletype_final_refined) %>% mutate(Normalised_mean_relative_abundance = Median_relative_abundance_rarefied/sum(Median_relative_abundance_rarefied)) %>% as.data.frame()

# Need a single entry for the Other group
LOI_group_1_summary.df <- LOI_group_1_summary.df %>% group_by(Length_of_immunosuppression_group_1, Sampletype_final_refined, taxonomy_label) %>% dplyr::summarise(Normalised_mean_relative_abundance = sum(Normalised_mean_relative_abundance)) %>% as.data.frame()
LOI_group_2_summary.df <- LOI_group_2_summary.df %>% group_by(Length_of_immunosuppression_group_2, Sampletype_final_refined, taxonomy_label) %>% dplyr::summarise(Normalised_mean_relative_abundance = sum(Normalised_mean_relative_abundance)) %>% as.data.frame()

# -------------------------
# Calculate the mean bacterial loads for each lesion type + suppression group


LOI_group_1_mean_bacterial_loads.df <- immunocompromised_data.df %>% group_by(Length_of_immunosuppression_group_1, Sampletype_final_refined) %>% dplyr::summarise(Mean_bacterial_load = mean(Bacterial_load_CFU, na.rm = T)) %>% as.data.frame()
rownames(LOI_group_1_mean_bacterial_loads.df) <- with(LOI_group_1_mean_bacterial_loads.df, paste0(Length_of_immunosuppression_group_1, "__", Sampletype_final_refined))
LOI_group_1_summary.df$Group <- with(LOI_group_1_summary.df, paste0(Length_of_immunosuppression_group_1, "__", Sampletype_final_refined))

LOI_group_2_mean_bacterial_loads.df <- immunocompromised_data.df %>% group_by(Length_of_immunosuppression_group_2, Sampletype_final_refined) %>% dplyr::summarise(Mean_bacterial_load = mean(Bacterial_load_CFU, na.rm = T)) %>% as.data.frame()
rownames(LOI_group_2_mean_bacterial_loads.df) <- with(LOI_group_2_mean_bacterial_loads.df, paste0(Length_of_immunosuppression_group_2, "__", Sampletype_final_refined))
LOI_group_2_summary.df$Group <- with(LOI_group_2_summary.df, paste0(Length_of_immunosuppression_group_2, "__", Sampletype_final_refined))

# -------------------------

# Calculate abundance value proportional to bacterial load
LOI_group_1_summary.df$Mean_relative_abundance_rarefied_BL_proportional <-
  apply(LOI_group_1_summary.df, 1, function(x) as.numeric(x["Normalised_mean_relative_abundance"]) * LOI_group_1_mean_bacterial_loads.df[x["Group"],"Mean_bacterial_load"])

LOI_group_2_summary.df$Mean_relative_abundance_rarefied_BL_proportional <-
  apply(LOI_group_2_summary.df, 1, function(x) as.numeric(x["Normalised_mean_relative_abundance"]) * LOI_group_2_mean_bacterial_loads.df[x["Group"],"Mean_bacterial_load"])

# Order taxonomy by the abundance. This is only approximate.
LOI_group_1_summary.df <- LOI_group_1_summary.df %>% group_by(Length_of_immunosuppression_group_1, Sampletype_final_refined) %>% arrange(Normalised_mean_relative_abundance) %>% as.data.frame()
...
LOI_group_2_summary.df <- LOI_group_2_summary.df %>% group_by(Length_of_immunosuppression_group_2, Sampletype_final_refined) %>% arrange(Normalised_mean_relative_abundance) %>% as.data.frame()
...
LOI_group_1_abundance_plot <- ggplot(LOI_group_1_summary.df, aes(x = Length_of_immunosuppression_group_1, y = Normalised_mean_relative_abundance*100, fill = taxonomy_label)) +
  ...
  common_theme + 
  facet_wrap(~Sampletype_final_refined, ncol = 1)

LOI_group_1_BL_abundance_plot <- ggplot(LOI_group_1_summary.df, aes(x = Length_of_immunosuppression_group_1, y = Mean_relative_abundance_rarefied_BL_proportional, fill = taxonomy_label)) +
  geom_bar(stat = "identity", colour = "black", lwd = .2) +
  coord_flip() +
  scale_fill_manual(values = genus_palette_lesions, guide = F) +
  scale_y_continuous(breaks = seq(0,60000, by = 5000), limits=c(0,60001)) +
  xlab("") +
...
grid_plot <- plot_grid(plotlist = list(LOI_group_1_abundance_plot, NULL, LOI_group_1_BL_abundance_plot),ncol = 3,nrow=1, rel_widths = c(1,-.12,1),align = "hv")
...
LOI_group_2_BL_abundance_plot <- ggplot(LOI_group_2_summary.df, aes(x = Length_of_immunosuppression_group_2, y = Mean_relative_abundance_rarefied_BL_proportional, fill = taxonomy_label)) +
  geom_bar(stat = "identity", colour = "black", lwd = .2) +
  coord_flip() +
  scale_fill_manual(values = genus_palette_lesions, guide = F) +
  scale_y_continuous(breaks = seq(0,60000, by = 5000), limits=c(0,60001)) +
...
grid_plot <- plot_grid(plotlist = list(LOI_group_2_abundance_plot, NULL, LOI_group_2_BL_abundance_plot),ncol = 3,nrow=1, rel_widths = c(1,-.12,1),align = "hv")
grid_plot <- plot_grid(grid_plot, my_legend_taxa, rel_heights = c(1,0.4), ncol = 1, nrow=2)
grid_plot
