...
otu_rel.m <- as.matrix(read.table(file = "Result_tables/OTU_relative_abundances.csv", sep = ",", header = T, row.names = 1))
otu_species_rel.m <- as.matrix(read.table(file = "Result_tables/Specie_relative_abundances.csv", sep = ",", header = T, row.names = 1))
otu_genus_rel.m <- as.matrix(read.table(file = "Result_tables/Genus_relative_abundances.csv", sep = ",", header = T, row.names = 1))
otu_family_rel.m <- as.matrix(read.table(file = "Result_tables/Family_relative_abundances.csv", sep = ",", header = T, row.names = 1))
otu_order_rel.m <- as.matrix(read.table(file = "Result_tables/Order_relative_abundances.csv", sep = ",", header = T, row.names = 1))
otu_class_rel.m <- as.matrix(read.table(file = "Result_tables/Class_relative_abundances.csv", sep = ",", header = T, row.names = 1))
otu_phylum_rel.m <- as.matrix(read.table(file = "Result_tables/Phylum_relative_abundances.csv", sep = ",", header = T, row.names = 1))


# Pick one
# otu, species, genus, family, 
choice = "species"

# heatmap.m <- otu_rel.m * 100 # * 100 to turn into percentages
# heatmap.m <- otu_species_rel.m * 100 # * 100 to turn into percentages
heatmap.m <- otu_genus_rel.m * 100 # * 100 to turn into percentages
# heatmap.m <- otu_family_rel.m * 100 # * 100 to turn into percentages
# heatmap.m <- otu_order_rel.m * 100 # * 100 to turn into percentages
# heatmap.m <- otu_class_rel.m * 100 # * 100 to turn into percentages
#heatmap.m <- otu_phylum_rel.m * 100 # * 100 to turn into percentages


# Determine the corresponding metadata group for the sample id in the OTU table
# Change the metadata variable as desired
my_groups <- c()
for (sample_id in colnames(heatmap.m)){
  sample_group <- paste(metadata[metadata$Index == sample_id,]$Sampletype)
  
  # Combination of two variables
  # sample_group <- paste(metadata[metadata$sample_name == sample_id,]$preservation, 
  #                       metadata[metadata$sample_name == sample_id,]$treatment, sep = ";")
  if (sample_group == ""){
