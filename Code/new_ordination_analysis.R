...
generate_pca <- function(pca_object, mymetadata, variable_to_plot, color_pallete, limits = NULL, filename = NULL, add_legend = F, add_spider = F, add_ellipse = F,
                         ...
                         legend_x = NULL, legend_y = NULL, plot_spiders = NULL, plot_ellipses = NULL,legend_cols = 2, legend_title = NULL,
                         label_ellipse = F, ellipse_label_size = 0.5,
                         show_legend = T){
  ...
  variable_colours <- setNames(color_pallete[1:length(variable_values)], variable_values)
...
# Subset the data for each cohort and create the corresponding ordinations  
metadata_immunocompetent.df <- subset(metadata.df, Project == "immunocompetent")
...
#"S4273r4_J607","S4273r2_J220"
metadata_immunocompromised.df <- subset(metadata.df, Project == "immunocompromised")
...
colnames(otu_rare_clr_filtered_compromised.m)[!rownames(metadata_immunocompromised.df) == colnames(otu_rare_clr_filtered_compromised.m)]

# pca analysis (same as PCA but option of distance; unconstrained ordination)
# PCA ojects for both, competent and compromised datasets
m.pca <- rda(t(otu_rare_clr_filtered.m), data = metadata.df)
...
generate_pca(m.pca, mymetadata = metadata.df,
             ...
             limits = c(-5,5,-5,5),
             point_size = .6, point_line_thickness = .3,point_alpha =.7,
             plot_spiders = F,
             plot_ellipses = F,
             label_ellipse = F, ellipse_label_size = .3,
             color_pallete = my_colour_pallete_206_distinct,
             variable_to_plot = "Patient", legend_cols = 1, show_legend = F,
             filename = "Result_figures/pcoa_dbrda_plots/both_cohorts_Patient_no_legend.pdf")

# Plot each variable for both cohorts 
for (myvar in variables_of_interest){
  out_name <- paste0("Result_figures/pcoa_dbrda_plots/both_cohorts_", myvar,".pdf")
  mylegendtitle <- myvar
  if (length(unique(metadata.df[,myvar])) > 10){
    mypal <- my_colour_pallete_206_distinct
  } else{
    mypal <- my_colour_pallete_10_distinct
  }
  generate_pca(m.pca, mymetadata = metadata.df,
               plot_height = 6, plot_width = 6,
               legend_x = -6, legend_y = 5, legend_cols = 1, legend_title = mylegendtitle,
               limits = c(-5,5,-5,5),
               point_size = .6, point_line_thickness = .1,point_alpha =.7,
               plot_spiders = F,
               plot_ellipses = F,
               label_ellipse = F, ellipse_label_size = .3,
               color_pallete = mypal,
...
# Plot each variable for immunocompetent
for (myvar in variables_of_interest){
  out_name <- paste0("Result_figures/pcoa_dbrda_plots/immunocompetent_", myvar,".pdf")
  mylegendtitle <- myvar
  if (length(unique(metadata.df[,myvar])) > 10){
...
# Plot each variable for immunocompromised
for (myvar in variables_of_interest){
  out_name <- paste0("Result_figures/pcoa_dbrda_plots/immunocompromised_", myvar,".pdf")
  mylegendtitle <- myvar
  if (length(unique(metadata.df[,myvar])) > 10){
    mypal <- my_colour_pallete_206_distinct
  } else{
...
adonis(t(otu_rare_clr_filtered.m)~Patient+Sampletype_pooled+Patient:Sampletype_pooled,data = metadata.df, permu=999,method="euclidean")
